...
class Board {
    ...
    boolean legalMove(Move move) {
        ...
        PieceColor og = _board[index(move.col0(), move.row0())];
        if (move.col1() < ('a') || move.col1() > ('g')) {
            return false;
        }
        char test1 = move.row1();
        int test2 = move.row1();
        int test3 = '7' + 1;
        if ((int) move.row1() < ('1') || (int) move.row1() > ('7')) {
            return false;
        }
    ...
    boolean checkPiece(char a, char b) {
        for (int i = -2; i < 3; i++) {
            for (int j = -2; j < 3; j++) {
                if (_board[neighbor(index(a, b), j, i)] == EMPTY) {
                    return true;
                }
            }
        }
    ...
    void convertNeighbors(char a, char b, PieceColor opponent) {
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                if (_board[neighbor(index(a, b), j, i)] == opponent) {
                    addUndo(neighbor(index(a, b), j, i));
                    _board[neighbor(index(a, b), j, i)] = _whoseMove;
    ...
    void makeMove(Move move) {
        ...
        if (move.isJump()) {
            ...
            convertNeighbors(move.col1(), move.row1(), opponent);
        } else {
            _numJumps = 0;
            addUndo(index(move.col1(), move.row1()));
            _board[index(move.col1(), move.row1())] = _whoseMove;
    ...
    void undo() {
        while (_undoPieces.peek() != null) {
            int curr = _undoSquares.pop();
            PieceColor old = _undoPieces.pop();
            boolean a = _board[curr] == BLUE;
            boolean b = _board[curr] == RED;
            if (a || b) {
                incrPieces(_board[curr], -1);
            }
            _board[curr] = old;
            incrPieces(old, 1);
        }
        if (_prevJumps.peek() != null) {
            _numJumps = _prevJumps.pop();
            _prevJumps.pop();
        }
        _undoSquares.pop();
        _undoPieces.pop();
    ...
    private void startUndo() {
        ...
        _prevJumps.push(null);
    }

    private void addJump() {
        _prevJumps.push(numJumps());
    }
    ...
    void setBlock(char c, char r) {
        ...
        char reflectCol = c;
        if (check1 > '4') {
            reflectRow = (char) ((check1 + '4'));
        } else if (check1 < '4') {
            reflectRow = (char) (('4' - check1) + '4');
        }
        if (check2 > 'd') {
            reflectCol = (char) (check2 + 'd');
        } else if (check2 < 'd') {
            reflectCol = (char) (('d' - check2) + 'd');
        }
        if (check1 != reflectRow) {
            _board[index(c, reflectRow)] = BLOCKED;
            _totalOpen -= 1;
        }
        if (check2 != reflectCol) {
            _board[index(reflectCol, r)] = BLOCKED;
            _totalOpen -= 1;
        }
        boolean a = check1 != reflectRow;
        boolean b = check2 != reflectCol;
        if (a && b) {
            _board[index(reflectCol, reflectRow)] = BLOCKED;
            _totalOpen -= 1;
        }
    ...
    /** Stack of jumps. */
    private Stack<Integer> _prevJumps;
